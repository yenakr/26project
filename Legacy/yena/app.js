...
// Theme Check to prevent flicker
const savedTheme = localStorage.getItem('calorieFitTheme');
if (savedTheme) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (themeToggleBtn) {
        ...
        if (icon) icon.className = currentTheme === 'dark' ? 'ri-sun-line' : 'ri-moon-fill';

        themeToggleBtn.addEventListener('click', () => {
            const tempTheme = document.documentElement.getAttribute('data-theme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
            const newTheme = tempTheme === 'dark' ? 'light' : 'dark';
            
            document.documentElement.setAttribute('data-theme', newTheme);
            localStorage.setItem('calorieFitTheme', newTheme);
            if (icon) icon.className = newTheme === 'dark' ? 'ri-sun-line' : 'ri-moon-fill';
        });
    }
